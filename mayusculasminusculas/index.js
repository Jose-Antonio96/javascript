//Definir una función que muestre información sobre una cadena de texto que se le pasa como argumento. 
//A partir de la cadena que se le pasa, la función determina si esa cadena está formada sólo por mayúsculas, sólo por minúsculas o por una mezcla de ambas.

let texto = prompt("introduzca una cadena de texto");

function mayusculasminusculas(texto) {
  if(texto == texto.uppercase) {
    return "Tiene mayúsculas";
  }
  else if(texto == texto.lowercase) {
    return "Tiene minúsculas"
  }
  else{
    if(texto == texto.uppercase && texto.lowercase){
      return "Tiene mayúsculas y minúsculas";
    }
  }
}

let resultado = mayusculasminusculas(texto);
